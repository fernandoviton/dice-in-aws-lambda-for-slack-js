...
// the result is an array with '3', '10', "+" (can be "+" or "-"), and '5' in that order (for the example given)
function parseDiceText(diceText)
{
    ...
    ret.push(match[1] !== "" ? parseInt(match[1],10) : 1); // the '3' in example above -> optional
...
// Savage Worlds specific parsing - if there is an S, returns a split array
...
//randomly selects a card from a deck of 54 (full deck plus two jokers)
function drawCard() {
...
//checks if there is anything listed after s and also checks if the user wants a card instead of dice rolls.
//otherwise runs multiple dice rolling function: rollMultiDice
...
//    console.log("data before rolling for savage: " + commandData)
    ...
    if (commandData[1] === "card") {
        ...
        response = rollMultiDice(roll, userName, commandData.splice(1), true)
    }
    return response
...
//loops through all the dice you want to roll and compiles a reponse
//breaks from all compiled responese for the first die that doesn't fit the format
function rollMultiDice(roll, userName, commandData, explode) {
    let response = ""
    let tempResponse = ""
    for (i=0; i < commandData.length; i++) {
        ...
        tempResponse = rollBasicFunction(roll, userName, commandData[i], explode)
...
//just returns an array split on one or more spaces. Should be a list of dice
function parseStandardRoll(text) {
    return text.split(/\s+/) 
}

//checks if you want to draw a card, otherwise calls the roll multipledice function
function rollStandardRoll(roll, userName, commandData) {
    //    console.log("data before rolling for standard: " + commandData)
    let response = ""
    if (commandData[0] === "card") {
        response = userName + " drew the " + drawCard()
    } else {
        response = rollMultiDice(roll, userName, commandData, false)

    }
    return response
}

//takes a single die command and compiles the total number and a response
...
//main function that slack calls
const handlerImpl = (roll, userName, text) =>
{
    ...
    if (text)
    {
        ...
        let responseB = rollStandardRoll(roll, userName, parseStandardRoll(text))
        // console.log("standard: " + responseB )
