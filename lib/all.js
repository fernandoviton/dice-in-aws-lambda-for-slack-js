...
function doRollForS(roll, commandData) {
    ...
    for (i=1; i < commandData.length; i++) {
        let rollData = commandData[i]
        console.log(rollData)

        //roll die, get results or error
        //if error, break out and say one or more of your dice was wrong
        //if result, great!
    }

    //let unmodifyRoll = rollDice(roll, roll)
...
function rollBasicFunction(roll, userName, text) {
        ...
        {
            var subtotalRoll = rollDice(roll, rollData[1], rollData[2]);
            let total = 0
            if (rollData[3] === "-") {
...
const handlerImpl = (roll, userName, text) =>
{
    if (text)
    {
        var responseS = doRollForS(roll, parseDiceTextForS(text));
        if (responseS !== null)
        {
            //return {result: {"response_type": "in_channel", "text" : userName + " " + responseS, "mrkdwn": true}};
            console.log("Savage World!")
