...
function doRollForC(rollData)
{
    ...
    var originalRoll = rollPercentiles();
    ...
    {
        var bonusDie = rollPercentiles()[0];
    ...
    {
        var penaltyDie = rollPercentiles()[0];
...
function roll4d6No1s()
{
    return Array.apply(null, {length: 4}).map(_ => roll(2,6));
}

function getNewChracterAttributeAsText()
{
    const roll = roll4d6No1s();
    const minItem = Math.min.apply(null, roll);
    const total = roll.reduce((sum, value) => sum + value) - minItem;
    const rollExplained = roll.toString();

    return rollExplained + '=*' + total +'*';
}

function doRollForNewCharacter(rollData)
{
    const startText = 'rolled (re-rolling 1s and dropping lowest):\n';
    const rollsAsText = Array.apply(null, {length: 6})
        .map(getNewChracterAttributeAsText)
        .reduce((sum, value) => sum + '\n' + value);

    return startText + rollsAsText;
}

exports.handlerImpl = (userName, text, doneFunc, errorFunc) =>
{
    if (text)
    {
        var responseC = doRollForC(parseDiceTextForC(text));
        if (responseC !== null)
        {
            doneFunc(null, {"response_type": "in_channel", "text" : userName + " " + responseC, "mrkdwn": true});
            return true;
        }

        //var responseNewCharacter = doRollForNewCharacter(parseDiceTextForNewCharacter(text));
        //if (responseNewCharacter !== null)
        //{
        //    doneFunc(null, {"response_type": "in_channel", "text" : userName + " " + responseNewCharacter, "mrkdwn": true});
        //    return;
        //}
        ...
        else
        {
            ...
            doneFunc(null, {"response_type": "in_channel", "text" : response, "mrkdwn": true});
            return true;
        }
    }
    else
    {
        errorFunc('Please specify dice');
        return false;
    }
}

exports.handler = function(event, context) {
    const succeeded = handlerImpl(event.user_name, event.text, context.done, context.error);
