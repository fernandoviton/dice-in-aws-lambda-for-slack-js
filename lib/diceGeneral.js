...
exports.textFromRollData = function(rollData)
{
  var n = rollData.numOfDice == undefined ? 1 : rollData.numOfDice;
  var s = rollData.sides == undefined ? 0 : rollData.sides;
  var a = rollData.add == undefined ? 0 : rollData.add;

  var ret = String(n) + "d" + String(s);
  if (a != 0)
    ret += "+" + String(a);
...
// given results of parseForGeneralDiceRoll translates into rollDice options (with additiona "add" property)
exports.getRollDataFromParsedRoll = function(parsedRollData)
{
  if (parsedRollData == null || parsedRollData.length < 3) return null;
